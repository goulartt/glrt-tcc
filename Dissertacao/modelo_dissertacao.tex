...
\nivel{Mestrado}
\titulacao{Bacharelado}
\area{Engenharia de Software}
\titulo{Modelo de Refatoração de Monolitos em Microserviços} % titulo do trabalho em português
\title{Model of Monolithic Refactoring in Microservices} % titulo do trabalho em inglês
\autor{João Victor Goulart de Almeida} % autor do trabalho
\cita{GOULART, João} % sobrenome (maiúsculas), nome do autor do trabalho
...
\coordenadorUTFPR[Coordenadora]{Grau}{Érica Ferreira De Souza} % Coordenador do PPGEEL
...
%%%%% abstract em ingles
\begin{abstract}
ALMEIDA, João Victor Goulart. \textbf{\UTFPRtitledata.} \UTFPRanodefesa. \pageref{LastPage} f. Undergraduate thesis -- Software Engineering Graduate Program, Federal University of Technology - Paraná. Cornélio Procópio, \UTFPRdatadata.

Over time corporate applications tend to grow greatly becoming large monoliths, the famous legacy systems. With this, some solutions were devised for this problem, the main one is to refactor the system in a microservice-based architecture. However, there are several complex challenges in the middle of the way, this work proposes a refactoring model that aims to minimize and avoid the maximum of these problems in order to achieve the final objective, which is to break this giant monolith into small microservices.

\textbf{Keywords:} Monolith. Microservices. Refactoring.
\end{abstract}

%%%%% abstract em frances
\begin{resume}
ALMEIDA, João Victor Goulart. \textbf{Modelo de Refatoração de Sistemas Monólitos em Microsserviços}. \UTFPRanodefesa. \pageref{LastPage} f. Proposta de Trabalho de	Conclusão de Curso (Graduação) ? Engenharia de Software. Universidade	Tecnológica Federal do Paraná. Cornélio Procópio, \UTFPRdatadata.

Com o tempo aplicações corporativas tendem a crescer muito se tornando grandes monólitos, os famosos sistemas legados. Com isso, foi idealizado algumas soluções para esse problema, a principal é refatorar o sistema em uma arquitetura baseada em microsserviços. Porém, existem diversos desafios complexos no meio do caminho, este trabalho propõe um modelo de refatoração que tem como objetivo amenizar e evitar o máximo desses problemas para que se consiga atingir objetivo final, que é quebrar esse monólito gigante em pequenos microsserviços.

\textbf{Palavras-chave:} Monólito. Microsserviços. Refatoração.
\end{resume}
